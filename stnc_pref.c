...
int client_performance_mode(char *ip, char *port, char *transferProtocol, char *transferParam, bool quietMode) {
	...
	switch(protocol)
	{
		case PROTOCOL_IPV4:
		{
			ret = perf_client_ipv4(data_to_send, chatSocket, FILE_SIZE, ip, (portNumber + 1), param, quietMode);
			break;
		}
		...
		case PROTOCOL_IPV6:
		{
			...
			char ipv6Address[] = "::1";
			ret = perf_client_ipv6(data_to_send, chatSocket, FILE_SIZE, ipv6Address, (portNumber + 1), param, quietMode);
			break;
		}
...
int server_performance_mode(char *port, bool quietMode) {
	...
	switch(protocol)
	{
		case PROTOCOL_IPV4:
		{
			actual_received = perf_server_ipv4(chatSocket, data_to_receive, fileSize, (portNumber + 1), param, quietMode);
			break;
		}

		case PROTOCOL_IPV6:
		{
			actual_received = perf_server_ipv6(chatSocket, data_to_receive, fileSize, (portNumber + 1), param, quietMode);
			break;
		}
...
int perf_client_ipv4(uint8_t* data, int chatsocket, uint32_t filesize, char *server_ip, uint16_t server_port, stnc_transfer_param param, bool quietMode) {
	...
	while (bytesSent < filesize)
	{
		...
		{
			if (!quietMode && (bytesSent % (CHUNK_SIZE * 16) == 0))
			...
			else
			{
				...
				int bytes = sendto(serverSocket, data + bytesSent, bytesToSend, 0, (struct sockaddr *)&serverAddress, len);
